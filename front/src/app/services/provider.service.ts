...
import { IDoctor, IPatient, IHospital, IAppointment, IMedicine, IAuthResponse } from '../interfaces/interfaces';
...
export class ProviderService extends MainService{
  ...
  createHospital(name: string,type: string, description: string, place: string,address: string): Promise<any>{
  ...
  createAppointment(name: string,type: string, description: string, doctor: IDoctor,patient: IPatient): Promise<any>{
  ...
  updateAppointment(appointment: IAppointment): Promise<IAppointment>{
    return this.put(`http://localhost:8000/api/appointments/${appointment.id}`, {
  ...
  createMedicine(name: string, company: string, cost: number, type: string, description: string): Promise<any>{
